...
To update the database use this command from the Web project folder (replace `Bytestrone.AppraisalSystem` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Bytestrone.AppraisalSystem.Infrastructure/Bytestrone.AppraisalSystem.Infrastructure.csproj -s Bytestrone.AppraisalSystem.Web.csproj
```
